...
const injectTawkTo = () => {
  if (document.getElementById('tawkto-script')) return; // already loaded
...
const DIRECT_CHAT_LINK = 'https://tawk.to/chat/6a8041ea1d62051d4f67fc2c/1k02g2vdd?layout=modern';
...
const SupportCenter = () => {
  ...
  useEffect(() => {
    // Initialize tawk.to API object before script loads
  ...
  const openLiveChat = () => {
    // Try to open widget if tawk is loaded, otherwise open direct link
    if (window.Tawk_API?.maximize) {
      window.Tawk_API.maximize();
    } else if (window.Tawk_API?.toggle) {
      window.Tawk_API.toggle();
    } else {
      window.open(DIRECT_CHAT_LINK, '_blank', 'noopener,noreferrer');
    }
  };
    ...
    <div style={{ padding: '80px 20px', minHeight: '100vh', background: '#000' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '56px' }}>
          <h1 style={{ fontSize: '52px', fontWeight: '900', color: '#fff', marginBottom: '16px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Support Center
          </h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: '#0d0d12', borderRadius: '16px', padding: '40px 32px', border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            ...
            <p style={{ color: '#a0a0b8', marginBottom: '24px', fontSize: '15px' }}>
              Contact our support team directly via email.
            </p>
            <a
              href="mailto:support@centennialinfotech.com"
              style={{ display: 'inline-block', background: 'rgba(168,85,247,0.15)', color: '#c084fc', textDecoration: 'none', padding: '12px 24px', borderRadius: '8px', fontWeight: '600', border: '1px solid rgba(168,85,247,0.35)', fontSize: '14px', transition: 'all 0.2s' }}
            >
          ...
          <div style={{ background: '#0d0d12', borderRadius: '16px', padding: '40px 32px', border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            ...
            <p style={{ color: '#a0a0b8', marginBottom: '8px', fontSize: '15px' }}>
              Chat with our team in real time for quick assistance.
            </p>
            <p style={{ color: '#6b6b88', fontSize: '13px', marginBottom: '24px' }}>
              Powered by tawk.to — the chat widget is active on this page.
            </p>
            <button
              onClick={openLiveChat}
              style={{ background: '#10b981', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: '8px', fontWeight: '700', cursor: 'pointer', fontSize: '15px', transition: 'opacity 0.2s' }}
          ...
          <div style={{ background: '#0d0d12', borderRadius: '16px', padding: '40px 32px', border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            ...
            <p style={{ color: '#6b6b88', fontSize: '13px', marginBottom: '24px' }}>
              Tickets are forwarded to <span style={{ color: '#a0a0b8' }}>{TICKET_EMAIL}</span>
            </p>
            <button
              onClick={openTicket}
              style={{ background: 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: '8px', fontWeight: '700', cursor: 'pointer', fontSize: '15px', transition: 'opacity 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.opacity = '0.85')}
              onMouseLeave={e => (e.currentTarget.style.opacity = '1')}
            >
              Create Ticket
